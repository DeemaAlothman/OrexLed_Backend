...
⚠️ **تحديث (14 أيلول 2026):** التسجيل هلق برقم الموبايل بدل الإيميل. الحسابات القديمة يلي كانت بالإيميل ما تأثرت — لسا فيهم يسجّلوا دخول عادي (شوفوا قسم تسجيل الدخول تحت).

```
POST /auth/register
Content-Type: application/json

{ "phone": "+963991234567", "password": "at-least-8-chars", "name": "الاسم" }
```
`phone` أي صيغة أرقام موبايل (بادئة + اختيارية، 7-15 رقم).

**نجاح (201):**
```json
{
  "user": { "id": "uuid", "email": null, "phone": "+963991234567", "role": "USER" },
  "accessToken": "eyJhbGciOi...",
  "refreshToken": "e057991c-0c7f-...-756b-426c-a07f-d2053cd4483a"
}
```
كل حساب جديد بينعمل بدور `USER` دايمًا وبرصيد **3 فيديوهات مجانية** تلقائيًا (تفاصيل الرصيد بمستند الفورمات المنفصل). ما فيه طريقة ترسلوا `role` بالـ body وتصيروا admin (السيرفر بيرفضه تلقائيًا بـ 400).

**أخطاء محتملة:**
- `400` — بيانات غير صالحة (رقم موبايل غلط الصيغة، باسورد أقل من 8 أحرف، اسم قصير...) مع تفاصيل الأخطاء بمصفوفة `message`
- `409` — رقم الموبايل مستخدم من قبل

### تسجيل الدخول

الحقل صار اسمه `identifier` بدل `email` — بيقبل **رقم موبايل** (حسابات جديدة) أو **إيميل** (حسابات قديمة سُجّلت قبل التحديث). السيرفر بيعرف تلقائيًا أيهم حسب وجود `@`.

```
POST /auth/login
Content-Type: application/json

{ "identifier": "+963991234567", "password": "..." }
```
أو للحسابات القديمة:
```
...
**فشل (401):** `{ "message": "Invalid credentials" }`
...
```
**نجاح (200):** `{ "id": "...", "email": "..." | null, "phone": "..." | null, "role": "USER" | "ADMIN" }`
**بدون توكن أو توكن منتهي:** `401`

للحصول على الرصيد (`videoCredits`) كمان، استخدموا `GET /users/me` بدل `/auth/me` — موثّق بمستند الفورمات المنفصل.
...
| `409` | تعارض (مثلاً رقم موبايل مسجل مسبقًا) |
...
## 9. ميزات جاهزة موثّقة بمستند منفصل

- فورمات الموقع (طلب عرض سعر، كن مندوبنا، مشاريعنا، طلب صيانة)
- توليد الفيديو بالذكاء الاصطناعي (Veo) + رصيد المستخدم
- بيانات الحساب الكاملة والرصيد (`GET /users/me`)

كل هاد بـ [FORMS_INTEGRATION.md](FORMS_INTEGRATION.md).
