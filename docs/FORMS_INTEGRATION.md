...
التوليد بياخد وقت (دقيقة تقريبًا)، فالتصميم **غير متزامن**: `POST` بيرجع فورًا بحالة `PENDING`، والتطبيق لازم يعمل **polling** على `GET /video-generations/:id` لحد ما تصير الحالة `COMPLETED` أو `FAILED`.

**✅ الخدمة شغالة فعليًا وموثّقة (14 أيلول 2026)** — الفوترة مفعّلة، والتوليد بيرجع فيديو حقيقي. مدة كل فيديو **8 ثوانٍ بالضبط دايمًا** (هاد أقصى حد فرضته Google نفسها على الموديل، ما فينا نتجاوزه). لو النص المطلوب توليده فيه كتابة عربية على الشاشة، في تحسين على مستوى الـ prompt من طرفنا يرفع احتمال ظهورها صح — بس مش ضمان 100% (محدودية معروفة بكل نماذج توليد الفيديو الحالية).
...
بدل ما تفتحوا `videoPath` مباشرة (بيشتغل inline بالمتصفح/الـ WebView)، فيه endpoint مخصّص للتحميل بيرجّع الملف كـ attachment (يفتح "حفظ باسم" أو تحميل مباشر بالتطبيق):

```
GET /video-generations/:id/download
Authorization: Bearer <accessToken>
```

نفس شرط الملكية تبع الـ polling (403 لطلب مستخدم تاني). لازم الحالة تكون `COMPLETED` وإلا بيرجع `400`.

### أخطاء محتملة

| كود | السبب |
|---|---|
| `400` | حقل ناقص/غير صالح، نوع صورة غير مدعوم، أو تحميل فيديو لسا مش `COMPLETED` |
| `401` | بدون توكن |
| `402` | رصيد الفيديوهات خلص — لازم شحن (شوفوا القسم 6) |
| `403` | حاولتوا تجيبوا/تحمّلوا طلب مستخدم تاني |
| `404` | الـ id مش موجود |
...
| توليد فيديو (تحميل) | `GET /video-generations/:id/download` | نفس المستخدم أو أدمن | — | — |
